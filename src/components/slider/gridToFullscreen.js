...
class GridToFullscreenEffect {
	...
	constructor( container, itemsWrapper, options = {} ) {
		...
		options.easings.toFullscreen =
			options.easings.toFullscreen || gsap.Power0.easeNone;
		options.easings.toGrid = options.easings.toGrid || gsap.Power0.easeNone;
	...
	init() {
		...
		const transformation = isFunction( this.options.transformation.type )
			? this.options.transformation.type(
...
const transformations = {
	...
	simplex: ( props ) => {
		...
		const amplitudeX = ensureFloat( props.amplitudeX || 0.5 );
		const amplitudeY = ensureFloat( props.amplitudeY || 0.5 );
		const frequencyX = ensureFloat( props.frequencyX || 1 );
		const frequencyY = ensureFloat( props.frequencyY || 0.75 );
